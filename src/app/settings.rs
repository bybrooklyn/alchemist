                    ...
                    {move || config.get().map(|cfg| {
                        cfg.map(|c| view! {
                            ...
                            <div class="glass-card">
                                <h2 class="text-xl mb-6 flex items-center gap-2">
                                    <svg class="w-5 h-5" style="color: rgb(var(--status-success))" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"></path></svg>
                                    "Library Folders"
                                </h2>
                                <div class="space-y-3">
                                    {c.scanner.directories.iter().map(|dir| view! {
                                        <div class="flex items-center gap-3 p-3 bg-slate-800 border border-slate-800 rounded-xl text-slate-400 font-mono text-sm">
