...
test("setup sidebar shows the full disabled app navigation", async ({ page }) => {
  ...
  for (const label of ["Dashboard", "Jobs", "Logs", "Statistics", "Intelligence", "Convert", "Settings"]) {
    await expect(page.locator("aside nav").getByText(label, { exact: true })).toBeVisible();
  }
  await expect(page.locator("aside nav")).toHaveClass(/pointer-events-none/);
  await expect(page.locator("aside nav")).toHaveClass(/opacity-40/);
});
...
test("setup polls hardware until the review step can complete", async ({ page }) => {
  let hardwareCalls = 0;
  let previewCalls = 0;
  ...
  await page.route("**/api/system/hardware", async (route) => {
    ...
    if (hardwareCalls < 8) {
      await fulfillJson(route, 503, {
        error: { message: "Hardware state unavailable" },
      });
      return;
    }
    await fulfillJson(route, 200, {
  ...
  await expect(page.getByText("Waiting for hardware detection to finish.")).toBeVisible();
  await expect(completeButton).toBeDisabled();
  await expect(completeButton).toBeEnabled({ timeout: 10000 });
});

test("setup surfaces preview failures inline and blocks leaving the library step", async ({ page }) => {
  await page.route("**/api/setup/status", async (route) => {
    await fulfillJson(route, 200, {
      setup_required: true,
      enable_telemetry: false,
  ...
  await expect(page.getByText("Preview path is not readable by the server.")).toBeVisible();
  await page.getByRole("button", { name: "Next" }).click();
  await expect(page.getByRole("heading", { name: "Library Selection" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "Processing" })).toHaveCount(0);
});
