...
- Login rate limiting keys on the direct socket peer IP. Behind a TLS-terminating reverse proxy this can rate-limit all clients together; configure `trusted_proxies` accordingly. Tracked as P2-33 in `audit.md`; fix planned for the next release.
- IPv4-mapped IPv6 peers (`::ffff:a.b.c.d`) are not normalized in LAN / trusted-proxy classification, which can affect dual-stack hosts. Tracked as RG-10 in `audit.md`.
