use std::{fs::File, num::NonZeroU64, path::PathBuf};
...
fn matroska_test_file(name: &str) -> PathBuf {
    let vendored = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../mkv-element/matroska-test-files/test_files")
        .join(name);
    if vendored.is_file() {
        vendored
    } else {
...
pub fn parse_test1_mkv() {
    let file = File::open(matroska_test_file("test1.mkv")).unwrap();
    let mut mkv = MatroskaFile::open(file).unwrap();
...
pub fn parse_test2_mkv() {
    let file = File::open(matroska_test_file("test2.mkv")).unwrap();
    let mut mkv = MatroskaFile::open(file).unwrap();
...
pub fn parse_test3_mkv() {
    let file = File::open(matroska_test_file("test3.mkv")).unwrap();
    let mut mkv = MatroskaFile::open(file).unwrap();
...
pub fn parse_test4_mkv() {
    let file = File::open(matroska_test_file("test4.mkv")).unwrap();
    let mut mkv = MatroskaFile::open(file).unwrap();
...
pub fn parse_test5_mkv() {
    let file = File::open(matroska_test_file("test5.mkv")).unwrap();
    let mut mkv = MatroskaFile::open(file).unwrap();
...
pub fn parse_test6_mkv() {
    let file = File::open(matroska_test_file("test6.mkv")).unwrap();
    let mut mkv = MatroskaFile::open(file).unwrap();
...
pub fn parse_test7_mkv() {
    let file = File::open(matroska_test_file("test7.mkv")).unwrap();
    let mut mkv = MatroskaFile::open(file).unwrap();
...
pub fn parse_test8_mkv() {
    let file = File::open(matroska_test_file("test8.mkv")).unwrap();
    let mut mkv = MatroskaFile::open(file).unwrap();
