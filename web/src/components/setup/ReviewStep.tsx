    ...
    return (
        <motion.div key="review" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                <ReviewCard title="Library" lines={[`${settings.scanner.directories.length} server folders selected`, preview ? `${preview.total_media_files} supported media files previewed` : "Preview pending", settings.scanner.directories.join(" | ") || "No folders selected"]} />
